...
public class AvroEntitySchema  extends EntitySchema {
     ...
     * @param rawSchema     The Avro Schema as a string that underlies the EntitySchema
     *                      implementation
     * @param fieldMappings The list of FieldMappings that specify how each field maps to an
     *                      HBase row
     */
    public AvroEntitySchema(Collection<String> tables, Schema schema, String rawSchema, Collection<FieldMapping> fieldMappings) {
    ...
    public boolean equals(Object obj) {
        ...
        if (schema == null) {
            ...
            for (Field field : schema.getFields()) {
                Field entitySchemaField = other.getAvroSchema().getFields().get(field.pos());
    ...
    public boolean compatible(AvroEntitySchema entitySchema) {
        return mappingCompatible(getRawSchema(), entitySchema.getRawSchema()) &&
               avroReadWriteSchemasCompatible(schema, entitySchema.getAvroSchema()) &&
               avroReadWriteSchemasCompatible(entitySchema.getAvroSchema(), schema);
    }
     ...
     */
    private static boolean mappingCompatible(String oldSchemaString, String newSchemaString) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Schemas not proper JSON in mappingCompatible", e);
        }
        ...
        for (JsonNode oldSchemaField : oldSchemaFields) {
            ...
            for (JsonNode newSchemaField : newSchemaFields) {
                if (oldSchemaFieldName.equals(newSchemaField.get("name").getTextValue())) {
                    if (!oldSchemaField.get("mapping").equals(newSchemaField.get("mapping"))) {
                        return false;
                    }
                }
            }
     ...
     */
    private static boolean avroReadWriteSchemasCompatible(Schema writer, Schema reader) {
        Symbol rootSymbol;
        try {
            ResolvingGrammarGenerator g = new ResolvingGrammarGenerator();
            rootSymbol = g.generate(writer, reader);
        } catch (IOException e) {
            throw new IllegalStateException("IOException while generating grammar.", e);
        }
     ...
     *
     * @param rootSymbol The root symbol to traverse from to look for an error symbol.
